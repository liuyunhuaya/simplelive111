...
class AccountController extends GetxController {
  ...
  void kuaishouTap() async {
    if (KuaishouAccountService.instance.hasCookie.value) {
      var result = await Utils.showAlertDialog("确定要清除快手Cookie吗？", title: "清除配置");
      if (result) {
        KuaishouAccountService.instance.clearCookie();
        SmartDialog.showToast("已清除快手Cookie");
      }
    } else {
      ...
      AlertDialog(
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              const Text(
                "快手直播部分功能需要登录后才能使用（如搜索、弹幕等）。\n请从浏览器登录快手后复制Cookie粘贴到此处。",
                style: TextStyle(fontSize: 12, color: Colors.grey),
              ),
              const SizedBox(height: 12),
              TextField(
          ...
          TextButton(
            onPressed: () {
              ...
              if (input.isEmpty) {
                ...
                KuaishouAccountService.instance.setCookie(input);
                SmartDialog.showToast("快手Cookie已保存");
              }
            },
            child: const Text("确定"),
          ),
